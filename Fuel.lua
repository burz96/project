...
Cdref("FDTank1Switch",         "FJS/727/Fuel/FDTank1Switch")
Cdref("FDTank2SwitchA",        "FJS/727/Fuel/FDTank2SwitchA")
Cdref("FDTank2SwitchB",        "FJS/727/Fuel/FDTank2SwitchB")
Cdref("FDTank3Switch",         "FJS/727/Fuel/FDTank3Switch")
Cdref("FDNozzleLSwitch",       "FJS/727/Fuel/FDNozzleLSwitch")
Cdref("FDNozzleRSwitch",       "FJS/727/Fuel/FDNozzleRSwitch")

Cdref("FDTank1Lite",           "FJS/727/Fuel/FDTank1Lite")
Cdref("FDTank2LiteA",          "FJS/727/Fuel/FDTank2LiteA")
Cdref("FDTank2LiteB",          "FJS/727/Fuel/FDTank2LiteB")
Cdref("FDTank3Lite",           "FJS/727/Fuel/FDTank3Lite")
Cdref("FDNozzleL_Lite",        "FJS/727/Fuel/FDNozzleL_Lite")
Cdref("FDNozzleR_Lite",        "FJS/727/Fuel/FDNozzleR_Lite")

local tank1DumpValve = 0
local tank2DumpValveA = 0
local tank2DumpValveB = 0
local tank3DumpValve = 0
local NozzleValveL = 0
local NozzleValveR = 0
...
function FuelDumpSystem()

    --control over the valuves
    if get(tank1Fuel) > 1587.57 then
        tank1DumpValve = tank1DumpValve + ((get(FDTank1Switch) - tank1DumpValve)*RR*2)
    else
        tank1DumpValve = tank1DumpValve + ((0 - tank1DumpValve)*RR*2)
    end
    if get(tank2Fuel) > 1587.57 then
        tank2DumpValveA = tank2DumpValveA + ((get(FDTank2SwitchA) - tank2DumpValveA)*RR*2)
        tank2DumpValveB = tank2DumpValveB + ((get(FDTank2SwitchB) - tank2DumpValveB)*RR*2)
    else
        tank2DumpValveA = tank2DumpValveA + ((0 - tank2DumpValveA)*RR*2)
        tank2DumpValveB = tank2DumpValveB + ((0 - tank2DumpValveB)*RR*2)
    end
    if get(tank3Fuel) > 1587.57 then
        tank3DumpValve = tank3DumpValve + ((get(FDTank3Switch) - tank3DumpValve)*RR*2)
    else
        tank3DumpValve = tank3DumpValve + ((0 - tank3DumpValve)*RR*2)
    end
        
    NozzleValveL = NozzleValveL + ((get(FDNozzleLSwitch) - NozzleValveL)*RR*2)
    NozzleValveR = NozzleValveR + ((get(FDNozzleRSwitch) - NozzleValveR)*RR*2)

    --dump fuel based on valve position
    if tank1DumpValve > 0.01 then
        set(dref_Fuel1, get(dref_Fuel1) - ((25.198566*tank1DumpValve*NozzleValveL)/(1/RR)))
    end
    if (tank2DumpValveA > 0.01 or tank2DumpValveB > 0.01) and get(tank2Fuel) > 1587.57 then
        local temp1 = NozzleValveL + NozzleValveR
        local temp2 = (tank2DumpValveA + tank2DumpValveB)*25.198566
        if temp1 > 1 then
            temp1 = 1
        end
        set(dref_Fuel2, get(dref_Fuel2) - ((temp2*temp1)/(1/RR)))
    end
    if tank3DumpValve > 0.01 and get(tank3Fuel) > 1587.57 then
        set(dref_Fuel3, get(dref_Fuel3) - ((25.198566*tank3DumpValve*NozzleValveR)/(1/RR)))
    end 
    
    
    -- set valuve position lights
    if tank1DumpValve > 0.01 and tank1DumpValve < 0.99 then
        set(FDTank1Lite, get(FDTank1Lite)+((1 - get(FDTank1Lite))*RR*20))
    else
        set(FDTank1Lite, get(FDTank1Lite)+((0 - get(FDTank1Lite))*RR*20))
    end
    if tank2DumpValveA > 0.01 and tank2DumpValveA < 0.99 then
        set(FDTank2LiteA, get(FDTank2LiteA)+((1 - get(FDTank2LiteA))*RR*20))
    else
        set(FDTank2LiteA, get(FDTank2LiteA)+((0 - get(FDTank2LiteA))*RR*20))
    end
    if tank2DumpValveB > 0.01 and tank2DumpValveB < 0.99 then
        set(FDTank2LiteB, get(FDTank2LiteB)+((1 - get(FDTank2LiteB))*RR*20))
    else
        set(FDTank2LiteB, get(FDTank2LiteB)+((0 - get(FDTank2LiteB))*RR*20))
    end
    if tank3DumpValve > 0.01 and tank3DumpValve < 0.99 then
        set(FDTank3Lite, get(FDTank3Lite)+((1 - get(FDTank3Lite))*RR*20))
    else
        set(FDTank3Lite, get(FDTank3Lite)+((0 - get(FDTank3Lite))*RR*20))
    end
    if NozzleValveL > 0.01 and NozzleValveL < 0.99 then
        set(FDNozzleL_Lite, get(FDNozzleL_Lite)+((1 - get(FDNozzleL_Lite))*RR*20))
    else
        set(FDNozzleL_Lite, get(FDNozzleL_Lite)+((0 - get(FDNozzleL_Lite))*RR*20))
    end
    if NozzleValveR > 0.01 and NozzleValveR < 0.99 then
        set(FDNozzleR_Lite, get(FDNozzleR_Lite)+((1 - get(FDNozzleR_Lite))*RR*20))
    else
        set(FDNozzleR_Lite, get(FDNozzleR_Lite)+((0 - get(FDNozzleR_Lite))*RR*20))
    end
