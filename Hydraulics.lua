...
function surfMove(Ratio, High, Low, clamp, trim, Tlim)
    local ratio2
    if clamp == 1 then
        if Ratio > 0.33 then
            Ratio2 = 0.33
...
dref_f("aileronTrim",                "sim/flightmodel2/controls/aileron_trim")
dref_f("RudderTrim",                 "sim/flightmodel2/controls/rudder_trim")
--local Atrimlim = 0.25
--local Rtrimlim = 0.303
...
function SurfaceMovements()
    ...
    set(INBDAileronLeft, surfMove(get(roll_ratio),22,-22, ailerClamp, get(aileronTrim), 0.25))
    set(INBDAileronRight, surfMove(get(roll_ratio),-22,22, ailerClamp, get(aileronTrim), 0.25))
    set(OUTBDAileronLeft, surfMove(get(roll_ratio), AileronUp,-AileronUp, ailerClamp, get(aileronTrim), 0.25))
    set(OUTBDAileronRight, surfMove(get(roll_ratio), -AileronUp,AileronUp, ailerClamp, get(aileronTrim), 0.25))
    set(ElevLeft, surfMove(get(pitch_ratio), -25,25, ElevClamp, 0, 0))
    set(ElevRight, surfMove(get(pitch_ratio), -25,25, ElevClamp, 0, 0))
    
    local RudMove = get(heading_ratio) + get(RudderTrim)*0.30303
    if RudMove > 1 then RudMove = 1 end
    if RudMove < -1 then RudMove = -1 end
    
    
    if get(HydPowRudSysB) == 1 then
        set(rudderUpper, myLerp( -1,0,1,0,RudMove)*(get(simHydPressure2)/3000 ) )
    else
        set(rudderUpper, myLerp( -1,-33,1,33,RudMove)*(get(simHydPressure2)/3000 ) )
    end
    if get(HydPowRudSysA) == 1 then
        set(rudderLower, myLerp( -1,0,1,0,RudMove)*(A_pressNeedle/3000 ) )
    else
        set(rudderLower, myLerp( -1,-33,1,33,RudMove)*(A_pressNeedle/3000 ) )
    end
