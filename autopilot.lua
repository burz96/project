...
Cdref("playSinkRate",            "FJS/727/autopilot/playSinkRate")
Cdref("playPullUp",              "FJS/727/autopilot/playPullUp")
Cdref("playDontSink",            "FJS/727/autopilot/playDontSink")
local dontsinkvar = 0
...
function GPWS()
    ...
    if get(dref_tireDef2) == 0 and tookOffTimer < 60 then
    ...
    local sinkenvol = myLerp(50,-1000,2500,-4500,get(dref_RadAlt))
    local pullenvol = myLerp(50,-1250,2500,-7000,get(dref_RadAlt))
    
    if get(dref_VVI) < sinkenvol and get(dref_VVI) > pullenvol and get(pow) == 1 then
        set(playSinkRate, 1)
    else
        set(playSinkRate, 0)
    end
    if get(dref_VVI) < pullenvol and get(pow) == 1 then
        set(playPullUp, 1)
    else
        set(playPullUp, 0)
    end
    
    if get(dref_RadAlt) < 200 then
        dontsinkvar = 1
    end
    if get(dref_RadAlt) > 1000 then
        dontsinkvar = 0
    end
        
    
    --if dontsinkvar == 1 and get(dref_RadAlt) > 50 and get(dref_RadAlt) < 700 and get(dref_VVI) < -250 and get(dref_landingGearRatio1) > 0 then
    --    set(playDontSink, 1)
    --else
    --    set(playDontSink, 0)
    --end
